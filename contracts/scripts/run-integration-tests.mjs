...
const integrationTestDir = path.join(contractsRoot, "test", "regular", "integration");
const tempRootDir = path.join(contractsRoot, ".codex-temp");
const compiledTestDir = path.join(contractsRoot, ".codex-temp", "integration-tests");
...
async function transpileTestFile(file) {
  ...
  const relativePath = path.relative(integrationTestDir, file);
...
async function main() {
  const files = await getIntegrationTestFiles();
  if (files.length === 0) {
    throw new Error(`No integration test files found in ${integrationTestDir}`);
  }
  ...
  try {
    for (const file of files) {
      ...
      const args = ["--test", "--test-concurrency=1", "--experimental-test-isolation=none", compiledFile];
