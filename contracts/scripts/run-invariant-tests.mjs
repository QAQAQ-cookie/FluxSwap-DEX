...
const invariantTestDir = path.join(contractsRoot, "test", "invariant");
...
async function getInvariantTestFiles() {
  const entries = await readdir(invariantTestDir, { withFileTypes: true });
  return entries
    .filter((entry) => entry.isFile() && entry.name.endsWith(".t.sol"))
    .map((entry) => path.join(invariantTestDir, entry.name))
...
async function main() {
  const files = await getInvariantTestFiles();
  if (files.length === 0) {
    throw new Error(`No invariant test files found in ${invariantTestDir}`);
  }
