...
const regressionTestDir = path.join(contractsRoot, "test", "regular", "regression");
const tempRootDir = path.join(contractsRoot, ".codex-temp");
const compiledTestDir = path.join(contractsRoot, ".codex-temp", "regression-tests");
...
async function collectRegressionTestFiles(dir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      // 回归测试按风险主题分子目录，这里递归收集所有 .ts 用例。
      files.push(...(await collectRegressionTestFiles(entryPath)));
      continue;
    }
...
async function transpileTestFile(file) {
  ...
  const relativePath = path.relative(regressionTestDir, file);
...
async function main() {
  const files = await collectRegressionTestFiles(regressionTestDir);
  if (files.length === 0) {
    throw new Error(`No regression test files found in ${regressionTestDir}`);
  }
  ...
  try {
    for (const file of files) {
      ...
      // 保持单文件串行执行，避免回归用例之间互相争抢 Hardhat 状态。
