...
const economicSecurityTestDir = path.join(contractsRoot, "test", "economic-security");
const tempRootDir = path.join(contractsRoot, ".codex-temp");
const compiledTestDir = path.join(tempRootDir, "economic-security-tests");
...
async function collectEconomicSecurityTestFiles(dir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      files.push(...(await collectEconomicSecurityTestFiles(entryPath)));
      continue;
    }
...
async function transpileTestFile(file) {
  ...
  const relativePath = path.relative(economicSecurityTestDir, file);
...
async function main() {
  const files = await collectEconomicSecurityTestFiles(economicSecurityTestDir);
  if (files.length === 0) {
    throw new Error(`No economic-security test files found in ${economicSecurityTestDir}`);
  }
