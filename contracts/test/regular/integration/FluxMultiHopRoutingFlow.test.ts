...
describe("FluxMultiHopRoutingFlow", async function () {
  ...
  it("should route a multi-hop swap across two pairs and accrue protocol fees for both input assets", async function () {
    ...
    const tokenAFee = (amountIn * protocolFeeBps) / feeBase;
    const tokenBFee = (amounts[1] * protocolFeeBps) / feeBase;
    const traderTokenCBefore = await tokenC.read.balanceOf([traderClient.account.address]);
    ...
    await router.write.swapExactTokensForTokens(
      [amountIn, 0n, path, traderClient.account.address, await getDeadline()],
      { account: traderClient.account.address }
    );

    strictEqual((await tokenC.read.balanceOf([traderClient.account.address])) - traderTokenCBefore, amounts[2]);
    ...
    strictEqual(await tokenC.read.balanceOf([treasury.address]), 0n);
