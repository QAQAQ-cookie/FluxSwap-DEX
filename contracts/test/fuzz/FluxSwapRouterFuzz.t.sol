...
contract FluxSwapRouterFuzzTest is Test {
    ...
    // Router fuzz 先覆盖最稳定、最核心的四条路径：
    // 1. token -> token 的 exact-input 交换，成交结果应与 quote 一致。
    // 2. token -> token 的 exact-output 交换，实际花费应与 quote 一致。
    // 3. 既有池子的 addLiquidity，不应超过用户给定的 desired 数量。
    // 4. ETH -> token 的 exact-input 交换，成交结果应与 quote 一致。
