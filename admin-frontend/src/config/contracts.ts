...
function resolveConfiguredChainId() {
  if (configuredChainId === hardhat.id || configuredChainId === sepolia.id) {
...
export const fluxChainId = resolveConfiguredChainId();
...
function compactAddressMap(
  ...
  const entries = Object.entries(addressMap).filter((entry): entry is [FluxContractName, Address] => {
    return typeof entry[1] === 'string' && entry[1].startsWith('0x');
  });
...
const requiredProtocolContracts: readonly FluxContractName[] = [
  'FluxBuybackExecutor',
...
export function getContractsForChain(chainId?: number | null): FluxContractAddressMap {
  if (chainId === undefined || chainId === null || chainId !== fluxChainId) {
    return {};
  }

  const generatedContractsByChain: Record<number, FluxContractAddressMap> = generatedContractAddressesByChain;

  return fluxChainId === hardhat.id ? generatedContractsByChain[hardhat.id] ?? {} : configuredContracts;
}
