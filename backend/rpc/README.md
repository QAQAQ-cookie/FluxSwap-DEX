...
这个目录是后端的 gRPC 接口层，负责接收前端或其他调用方请求，并完成参数校验、签名校验、落库、查单、撤单登记和找路报价等工作。
...
- `triggerPriceX18` 不是直接用两个最小单位整数做 `minAmountOut / amountIn`，而是要先按输入、输出代币精度归一后再放大到 `1e18`
- RPC 只校验并存储这些整数值，不负责把用户展示小数再转换一遍

执行奖励字段说明：

- `maxExecutorRewardBps` 表示执行器最多可以从 surplus 中拿走的比例
...
- 支持直连和单次中转两类路径
- 请求入参里的原生币语义使用零地址表示，进入后端后会映射到当前链的 `WETH`
- 路径搜索基于 Redis 中维护的交易对图谱元数据
- 每条候选路径都会通过链上 router 报价
- 最终使用 gas-adjusted quote 排序，返回已选路径和备选路径

当前 `GetBestRouteRequest` 入参：
...
当前 `GetBestRouteResponse` 重点字段：

- `selectedRoute`
- `alternativeRoutes`
- `execution.routerPath`
- `selectionReason`
- `usedGasAdjustedRanking`
...
- `internal/logic/getbestroutelogic_test.go`
